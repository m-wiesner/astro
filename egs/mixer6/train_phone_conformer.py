...
def main(rank, world_size, args):
    ...
    cuts_train_intv = load_manifest_lazy(
        f"{args.datadir}/mixer6_cuts_train_intv.jsonl.gz"
    )
    cuts_train_call = load_manifest_lazy(
        f"{args.datadir}/mixer6_cuts_train_call.jsonl.gz"
    )
    cuts_dev = load_manifest_lazy(
        f"{args.datadir}/mixer6_cuts_dev_a.jsonl.gz"
    )

    cuts_train = cuts_train_intv + cuts_train_call
    ...
    if args.resume is not None:
        ...
        args.start_epoch = params_dict['epoch'] + 1
    ...
    if args.resume is not None:
        optim.load_state_dict(params_dict['optimizer'])
        ...
        lr_sched.load_state_dict(params_dict['lr_sched'])
        if args.fp16:
            scaler.load_state_dict(params_dict['scaler'])
    else:
        lr_sched = torch.optim.lr_scheduler.OneCycleLR(
            optim, max_lr=args.lr,
            steps_per_epoch=sum(1 for b in train_dloader.sampler),

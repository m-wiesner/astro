...
def get_args():
    ...
    parser.add_argument('--lm-data', type=str, help='the data for training the lm')
...
def main(args): 
    ...
    cuts = load_manifest_lazy(
        f"{args.datadir}/mixer6_cuts_{args.part}.jsonl.gz"
    )

    if args.channel > -1:
        cuts = cuts.filter(lambda x: x.channel == args.channel)
    # Create the decoder
